...
- `alpha::Float64`:     Constant for diffusivity at top canopy. Default: `1.5` following Van de Griend et al (1989).
- `ZW::Float64`:        Top height of the roughness sublayer (m). Default: `ZPD + alpha * (Z2 - ZPD)`
...
```
"""
function GetWind(;Wind,LAI_lay,LAI_abv,Z_top,ZHT,extwind=0,Z0=Z_top*0.1,ZPD=Z_top*0.75,alpha=1.5,ZW=ZPD+alpha*(Z_top-ZPD),vonkarman=constants().vonkarman)
    if ZHT < Z_top
...
- `Z0::Float64`:        Roughness length (m). Default: `0.1*Z_top`
- `ZPD::Float64`:       Zero-plane displacement (m), Default: `0.75*Z_top`
- `alpha::Float64`:     Constant for diffusivity at top canopy. Default: `1.5` following
                 Van de Griend et al (1989).
- `ZW::Float64`:        Top height of the roughness sublayer (m). Default: `ZPD+alpha*(Z_top-ZPD)`
- `extwind::Float64`:   Extinction coefficient. Default: `0`, no extinction.
- `vonkarman::Float64`: Von Karman constant, default to `constants().vonkarman`, 0.41.
...
`alpha` can also be computed as: ```alpha=\\frac{zw-d}{Z_{top}-d}```  
...
```
"""
function G_bulk(;Wind,LAI,ZHT,Z_top,Z0=Z_top*0.1,ZPD=Z_top*0.75,alpha=1.5,ZW=ZPD+alpha*(Z_top-ZPD),extwind=0.0,vonkarman=constants().vonkarman)
    if ZHT < Z_top
        printstyled("Warning: Measurement height lower than canopy height (ZHT < Z_top), forcing ZHT > Z_top \n", bold= true,
                    color= :yellow)
        ZHT= 1.01*Z_top
    ...
    U_inter= GetWind(Wind= Wind,LAI_lay=0.0,LAI_abv=LAI/2.0,extwind=extwind,Z_top=Z_top,ZHT=ZHT,Z0=Z0,ZPD=ZPD,alpha=alpha,ZW=ZW)
...
```
"""
function Gb_h(;Wind,wleaf,LAI_lay,LAI_abv,extwind,Z_top,ZHT,Z0=Z_top*0.1,ZPD=Z_top*0.75,alpha=1.5,ZW=ZPD+alpha*(Z_top-ZPD))
    U_z= GetWind(Wind= Wind,LAI_lay= LAI_lay, LAI_abv= LAI_abv,extwind= extwind,Z_top= Z_top, ZHT= ZHT, Z0= Z0, ZPD= ZPD,alpha= alpha, ZW= ZW)
    0.01 * sqrt(U_z/wleaf)
end